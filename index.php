<?php  
require 'includes/app.php';

// estaAutenticado();

if (!isset($_SESSION)) {
    session_start();
}

// session_start();

// echo '<pre>';
// var_dump($_SESSION);
// echo '</pre>';

// $auth = $_SESSION['login'] ?? null ;

// if (!$auth) {
//     header('Location: /login');
// }

// echo '<pre>';
// var_dump($_POST);
// echo '</pre>';

incluirTemplate('header');
   
?>

<main class="l-main">
    <!--========== HOME ==========-->

    <section class="home" id="home">
        <div class="home__container bd-container bd-grid">
            <div class="home__img">
                <img src="build/img/tum1.webp" alt="">
            </div>

            <div class="home__data">
                <h1 class="home__title">R <span>Total</span></h1>
                <p class="home__description">Marca numero 1# en Gorras</p>

            </div>
        </div>
    </section>

    <!--========== DECORATION ==========-->
    <section class="decoration section bd-container" id="decoration">
        <h2 class="section-title">Lo más <br>Vendido</h2>
        <div class="decoration__container bd-grid">
            <div class="decoration__data">
                <img src="build/img/tum2.webp" alt="" class="decoration__img">
                <h3 class="accessory__title">Christmas Bells</h3>
                <a href="#" class="button button-link">Go Shopping</a>
            </div>

            <div class="decoration__data">
                <img src="build/img/tum2.webp" alt="" class="decoration__img">
                <h3 class="accessory__title">Christmas Candies</h3>
                <a href="#" class="button button-link">Go Shopping</a>
            </div>

            <div class="decoration__data">
                <img src="build/img/tum2.webp" alt="" class="decoration__img">
                <h3 class="accessory__title">Christmas Trees</h3>
                <a href="#" class="button button-link">Go Shopping</a>
            </div>
        </div>
    </section>

    <!--========== ACCESSORIES ==========-->
    <section class="accessory section bd-container" id="accessory">
        <h2 class="section-title">Has tu <br>Paquete </h2>

        <div class="accessory__container bd-grid">
            <div class="accessory__content">
                <img src="build/img/tum2.webp" alt="" class="accessory__img">
                <h3 class="accessory__title">Snow Globe</h3>
                <span class="accessory__category">Accessory</span>
                <span class="accessory__preci">$9.45</span>
                <a href="#" class="button accessory__button"><i class='bx bx-plus'></i></a>
            </div>

            <div class="accessory__content">
                <img src="build/img/tum2.webp" alt="" class="accessory__img">
                <h3 class="accessory__title">Candy</h3>
                <span class="accessory__category">Accessory</span>
                <span class="accessory__preci">$2.52</span>
                <a href="#" class="button accessory__button"><i class='bx bx-plus'></i></a>
            </div>

            <div class="accessory__content">
                <img src="build/img/tum2.webp" alt="" class="accessory__img">
                <h3 class="accessory__title">Angel</h3>
                <span class="accessory__category">Accessory</span>
                <span class="accessory__preci">$13.15</span>
                <a href="#" class="button accessory__button"><i class='bx bx-plus'></i></a>
            </div>

            <div class="accessory__content">
                <img src="build/img/tum2.webp" alt="" class="accessory__img">
                <h3 class="accessory__title">Sphere</h3>
                <span class="accessory__category">Accessory</span>
                <span class="accessory__preci">$4.25</span>
                <a href="#" class="button accessory__button"><i class='bx bx-plus'></i></a>
            </div>

            <div class="accessory__content">
                <img src="build/img/tum2.webp" alt="" class="accessory__img">
                <h3 class="accessory__title">Surprise gift</h3>
                <span class="accessory__category">Accessory</span>
                <span class="accessory__preci">$7.99</span>
                <a href="#" class="button accessory__button"><i class='bx bx-plus'></i></a>
            </div>
        </div>
    </section>

    <!--========== BRANDS or share or Marcas ==========-->
    <section class="share section bd-container" id="share">
        <div class="share__container bd-grid">
            <div class="share__data">
                <h2 class="section-title-center">Sharing Is The Best Gift <br> You Can Give</h2>
                <p class="share__description">Sharing these holidays is the best gift you can give, give a present
                    or share your love with the people you love the most and celebrate with great happiness.</p>
                <a href="#" class="button">Send a Gift</a>
            </div>

            <div class="share__img">
                <a>
                    <img src="build/img/tum2.webp" alt="">
                </a>
            </div>
        </div>
    </section>

    <!--========== SEND ==========-->
    <section class="send section" id="send">
        <div class="send__container bd-container bd-grid">
            <div class="send__content">
                <h2 class="section-title-center send__title">Newsletter</h2>
                <p class="send__description">Unete para resivir promociones y descuentos exclusivos</p>
                <form action="">
                    <div class="send__direction">
                        <input type="text" placeholder="House address" class="send__input">
                        <a href="#" class="button">Send</a>
                    </div>
                </form>
            </div>

            <div class="send__img">
                <img src="build/img/send.webp" alt="">
            </div>
        </div>
    </section>
</main>



<?php incluirTemplate('footer'); ?>